...
class CloudModel: BaseModel {
    ...
    var bluenoiseTexture: texture_t

    init(mesh: Mesh, shaderName: String, texture: texture_t, texture2: texture_t) {
        let scale = SIMD3<Float>(10.0, 10.0, 10.0)
        modelMatrix = float4x4.translation(SIMD3<Float>(25.0, 10.0, 0.0)) * float4x4.scale(scale)
...
//        super.init(mesh: mesh, shaderName: shaderName, texture: nil)
        self.bluenoiseTexture = texture2
...
//                    }
    ...
    override func draw() {
        ...
        glBindTexture(bluenoiseTexture.type, bluenoiseTexture.ID)
